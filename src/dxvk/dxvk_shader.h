...
public:
  DxvkShader(const DxvkShaderCreateInfo &info, SpirvCodeBuffer &&spirv);
   ...
   * \returns Shader info
   */
  const DxvkShaderCreateInfo &info() const { return m_info; }

  /**
   * \brief Retrieves shader flags
   * \returns Shader flags
   */
  DxvkShaderFlags flags() const { return m_flags; }

  /**
   * \brief Retrieves binding layout
   * \returns Binding layout
   */
  const DxvkBindingLayout &getBindings() const { return m_bindings; }

  /**
   * \brief Retrieves spec constant mask
   * \returns Bit mask of used spec constants
   */
  uint32_t getSpecConstantMask() const { return m_specConstantMask; }
   ...
   */
  bool needsLibraryCompile() const { return m_needsLibraryCompile.load(); }
   ...
   * \c false.
   */
  void notifyLibraryCompile() { m_needsLibraryCompile.store(false); }

  /**
   * \brief Gets raw code without modification
   */
  SpirvCodeBuffer getRawCode() const { return m_code.decompress(); }
   ...
   */
  SpirvCodeBuffer getCode(const DxvkBindingLayoutObjects *layout,
   ...
   */
  DxvkShaderKey getShaderKey() const { return m_key; }
   ...
   */
  std::string debugName() const { return m_key.toString(); }
...
private:
  ...
  static void emitOutputSwizzles(SpirvCodeBuffer &code, uint32_t outputMask,
                                 const VkComponentMapping *swizzles);

  static void emitFlatShadingDeclarations(SpirvCodeBuffer &code,
                                          uint32_t inputMask);

  static void patchInputTopology(SpirvCodeBuffer &code,
...
public:
   ...
   */
  void addStage(VkShaderStageFlagBits stage, SpirvCodeBuffer &&code,
   ...
   * \param [in] specinfo Specialization info
   */
  void addStage(VkShaderStageFlagBits stage,
...
struct DxvkShaderIdentifierSet {
  VkShaderModuleIdentifierEXT vs = {
      VK_STRUCTURE_TYPE_SHADER_MODULE_IDENTIFIER_EXT};
  VkShaderModuleIdentifierEXT tcs = {
      VK_STRUCTURE_TYPE_SHADER_MODULE_IDENTIFIER_EXT};
  VkShaderModuleIdentifierEXT tes = {
      VK_STRUCTURE_TYPE_SHADER_MODULE_IDENTIFIER_EXT};
  VkShaderModuleIdentifierEXT gs = {
      VK_STRUCTURE_TYPE_SHADER_MODULE_IDENTIFIER_EXT};
  VkShaderModuleIdentifierEXT fs = {
      VK_STRUCTURE_TYPE_SHADER_MODULE_IDENTIFIER_EXT};
  VkShaderModuleIdentifierEXT cs = {
      VK_STRUCTURE_TYPE_SHADER_MODULE_IDENTIFIER_EXT};
};
...
public:
   ...
   */
  void addShader(const Rc<DxvkShader> &shader);
...
public:
  DxvkShaderPipelineLibrary(const DxvkDevice *device,
   ...
   */
  VkShaderModuleIdentifierEXT getModuleIdentifier(VkShaderStageFlagBits stage);
...
private:
  ...
  compileShaderPipeline(VkPipelineCreateFlags flags);

  VkPipeline compileVertexShaderPipeline(const DxvkShaderStageInfo &stageInfo,
                                         VkPipelineCreateFlags flags);

  VkPipeline compileFragmentShaderPipeline(const DxvkShaderStageInfo &stageInfo,
                                           VkPipelineCreateFlags flags);

  VkPipeline compileComputeShaderPipeline(const DxvkShaderStageInfo &stageInfo,
                                          VkPipelineCreateFlags flags);

  SpirvCodeBuffer getShaderCode(VkShaderStageFlagBits stage) const;

  void generateModuleIdentifierLocked(VkShaderModuleIdentifierEXT *identifier,
                                      const SpirvCodeBuffer &spirvCode);

  VkShaderStageFlags getShaderStages() const;

  DxvkShader *getShader(VkShaderStageFlagBits stage) const;

  VkShaderModuleIdentifierEXT *getShaderIdentifier(VkShaderStageFlagBits stage);
